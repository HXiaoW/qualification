...
Starting with running the output notebook in a few ways, by both vscode directly or command line in terminal it all leads to a crash in kernel. I also went through the output log to see if there are any useful results, but I was only able to see the analysis planning, but not any output.  

I then tried running from command line to first save the output notebook into a script, then running it to a txt output file, however, the output also did not correctly ran, and the running was terminated before any output. During this time, I continously tried different method while editing the markdown files, but as of right now, I was not able to see any actual output or the agent interpretation. This was estimately where I paused as I am approaching the deadline. I tried debugging this for a few hours and emailed Matthew. 